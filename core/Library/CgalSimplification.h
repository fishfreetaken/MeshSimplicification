...
typedef boost::graph_traits<MyMesh>::edge_descriptor edge_descriptor;
typedef boost::graph_traits<MyMesh>::edge_iterator edge_iterator;
...
public:
	...
	Constrained_edge_map(MyMesh& sm)
		: sm_(sm)
	{
...
class CgalSimplification
{
public:
	CgalSimplification(MyMesh&sm);
	~CgalSimplification();

	void MeshSimplication(double x);
